...
-- Return true p is true for every v in s, otherwise false
-- set_forall: (p: a -> bool) -> (s: set a) -> bool
set_forall evenb (mkSet nil) = true  --p=evenb
...
-- Return true if p is true for some v in s, otherwise else false
-- set_exists: (p: a -> bool) -> (s: set a) -> bool
set_exists oddb (mkSet nil) = false --p=oddb--pairlist, (h l),return (h ei),ei belongs to l
...
set_witness p (mkSet (h::t)) = ite (set_exists p (mkSet (h::t)))
                              ((ite (p h)) (some h) (set_witness p (mkSet t)))
                              (none)
...
settoString: (Serialize a) => set a -> String
settoString (mkSet nil) = ""
settoString (mkSet (h::nil))= (toString h)
settoString (mkSet (h::t)) = (toString h) ++ ", " ++(settoString (mkSet t))
set_toString (mkSet l) = "(" ++ (settoString (mkSet l)) ++ ")"
-------------------------------------
