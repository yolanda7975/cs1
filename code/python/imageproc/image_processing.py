...
# return the color add or substrate of the given pixel
def intensify_new(pixel,delta):
    pixel_tmp=pixel
    if(pixel[0]+delta>255):
        pixel_tmp[0]=255
    elif(pixel[0]+delta<0):
        pixel_tmp[0]=0
    else:
        pixel_tmp[0]=pixel[0]+delta

    if(pixel[1]+delta>255):
        pixel_tmp[1]=255
    elif(pixel[1]+delta<0):
        pixel_tmp[1]=0
    else:
        pixel_tmp[1]=pixel[1]+delta

    if(pixel[2]+delta>255):
        pixel_tmp[2]=255
    elif(pixel[2]+delta<0):
        pixel_tmp[2]=0
    else:
        pixel_tmp[2]=pixel[2]+delta
    return (pixel_tmp[0],pixel_tmp[1], pixel_tmp[2])
...
# lightening: modifies image pixel array of image_surf in place
# replace each pixel with its photographic lightened image 
#
def lightening(image_surf):
    ...
    for x in range(rows):
        for y in range(cols):
            pixels3d[x,y] = intensify_new(pixels3d[x,y],10)
# darkening: modifies image pixel array of image_surf in place
# replace each pixel with its photographic darkened image 
#
def darkening(image_surf):
